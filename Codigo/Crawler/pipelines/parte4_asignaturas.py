...
def resolve_candidate_subject_guide_urls(
    ...
    if domain:
        ...
        # Filtrar subdominios verificados en el índice de descubrimiento para evitar
        # peticiones 404 a subdominios inexistentes (p. ej. secretaria., guiasdocentes., sia.)
        verified_subdomains = {
            host for host in discovered_host_counts
            if host != learned_public_host and is_same_or_subdomain(f"https://{host}", f"https://{clean_domain}")
        }
        for sub in ("guias", "guies", "guiasdocentes", "asignaturas", "docencia", "centros", "facultades", "secretaria", "sia"):
            cand_sub = f"{sub}.{clean_domain}"
            if cand_sub in discovered_host_counts:
                verified_subdomains.add(cand_sub)
        if clean_domain == "uca.es":
            verified_subdomains.add(f"asignaturas.{clean_domain}")

        # 3.1. Si solo hay slug sin código de asignatura
        if slug and not code_variants:
            candidate_limit = min(
                candidate_limit,
                max(1, MAX_SUBJECT_GUIDE_NO_CODE_CANDIDATES),
            )
            guide_domains = [learned_public_host] + sorted(verified_subdomains)
        ...
        # 3. Patrones institucionales genéricos: solo en el host verificado o subdominios descubiertos
        for c_code in code_variants:
            _add_url(f"https://{learned_public_host}/docencia/guia/{c_code}")
            _add_url(f"https://{learned_public_host}/guias/{c_code}")
            _add_url(f"https://{learned_public_host}/guias-docentes/{c_code}")
            _add_url(f"https://{learned_public_host}/guia.php?cod={c_code}")
            _add_url(f"https://{learned_public_host}/asignatura.php?id={c_code}")

            if f"secretaria.{clean_domain}" in verified_subdomains:
                _add_url(f"https://secretaria.{clean_domain}/docencia/guia/{c_code}")
            if f"cv1.cpd.{clean_domain}" in verified_subdomains:
                _add_url(f"https://cv1.cpd.{clean_domain}/ConsPlanesEstudio/cvFichaAsigRedir.asp?asig={c_code}")
            if f"guias.{clean_domain}" in verified_subdomains:
            ...
            if f"guiasdocentes.{clean_domain}" in verified_subdomains:
                _add_url(f"https://guiasdocentes.{clean_domain}/asignatura/{c_code}")
            if f"guies.{clean_domain}" in verified_subdomains:
                _add_url(f"https://guies.{clean_domain}/guia.php?asig={c_code}")
            if f"sia.{clean_domain}" in verified_subdomains:
                _add_url(f"https://sia.{clean_domain}/rest/publicacion/estudio/asignatura/{c_code}")

            for v_sub in verified_subdomains:
                for year in year_candidates:
                    _add_url(f"https://{v_sub}/{year}/{c_code}")
...
def _guide_quality_is_publishable(guide: dict, expected_name: str = "", expected_code: str = "", source_url: str = "") -> bool:
    """Acepta una guía solo con identidad y cobertura verificadas."""
    if not isinstance(guide, dict):
        return False
    quality = guide.get("calidad_extraccion")
    if not isinstance(quality, dict):
        quality = assess_subject_guide_quality(
            guide,
            expected_name=expected_name,
            expected_code=expected_code,
            source_url=source_url,
        )
        guide["calidad_extraccion"] = quality
    identity = quality.get("identidad") or {}
    return bool(quality.get("publicable") and identity.get("verificada"))
...
def _process_single_university_guides(
    ...
    for item in degree_items:
        ...
        for elem in elementos:
            ...
            if cached_data and not _guide_quality_is_publishable(
                cached_data, expected_name=asig_name, expected_code=asig_code, source_url=url_directa or ""
            ):
                cached_data = None
                stats["guide_identity_rejected"] += 1
            ...
            cache_is_negative = getattr(cache, "is_negative", None)
            for c_url in candidate_urls:
                if (
                    negative_registry.contains(c_url)
                    or c_url in run_negative_urls
                    or (
                        not revalidate_sources
                        and callable(cache_is_negative)
                        and cache_is_negative(c_url)
                    )
                ):
                ...
                if not revalidate_sources:
                    ...
                    if cached_candidate and _subject_guide_identity_matches(
                        ...
                        cached_candidate,
                        source_url=c_url,
                    ) and _guide_has_content(cached_candidate) and _guide_quality_is_publishable(
                        cached_candidate, expected_name=asig_name, expected_code=asig_code, source_url=c_url
                    ):
                ...
                try:
                    ...
                    if status_code == 200:
                        ...
                        if not _guide_quality_is_publishable(
                            parsed_guide,
                            expected_name=asig_name,
                            expected_code=asig_code,
                            source_url=c_url,
                            ...
                            stats["guide_identity_rejected"] += 1
                            elem["guia_docente_candidata"] = parsed_guide
                            elem["estado_guia_docente"] = "pendiente_revision"
                            elem["fecha_ultima_comprobacion_guia"] = datetime.now().isoformat()
                            degree_modified = True
                            continue
        ...
        if degree_modified:
            # La fase de guías puede cargar un plan que ya existía antes de
            # una nueva comprobación web. Recalcular aquí evita conservar una
            # etiqueta de calidad antigua incompatible con la fuente actual.
            current_plan = data.get("plan_estudios")
            if isinstance(current_plan, dict) and current_plan:
                current_quality = assess_plan_quality(data, data.get("origen_fuente"))
                data["calidad_datos"] = current_quality
                data["estado_ultima_extraccion"] = current_quality["estado"]
                data["estado_calidad"] = current_quality["estado"]
    ...
    try:
        ...
        for p_path in plan_files:
            ...
            if degree_level_filter and not matches_academic_level(
                data.get("titulo"), data.get("nivel_academico"), degree_level_filter
            ):
                continue
