...
BASE_DIR = os.getenv("CRAWLER_BASE_DIR", os.path.dirname(_DIR_PATH) if os.path.basename(_DIR_PATH) == "core" else _DIR_PATH)
...
HTTP_429_DEFAULT_RETRY_AFTER = _safe_int("CRAWLER_429_RETRY_AFTER", 65) # Retardo fallback para HTTP 429 (65s)
...
# Las respuestas de error de robots se cachean solo durante un intervalo corto
# para evitar repetir peticiones fallidas en una misma campaña, manteniendo la
# posibilidad de recuperar un origen cuando vuelva a estar disponible.
ROBOTS_ERROR_CACHE_TTL_SECONDS = _safe_int("CRAWLER_ROBOTS_ERROR_CACHE_TTL", 300)
# Un bloqueo persistido no es una verdad permanente: robots.txt y la
# disponibilidad del origen pueden cambiar. Tras este TTL se permite una
# nueva comprobación, sin convertir la ejecución incremental en una
# revalidación completa.
ROBOTS_DENIED_RETRY_TTL_SECONDS = _safe_int(
    "CRAWLER_ROBOTS_DENIED_RETRY_TTL",
    str(24 * 3600),
)
...
# Un presupuesto de quince minutos por titulación convierte una campaña
# nacional en impracticable: las rutas que no aportan currículo agotan el
# tiempo disponible antes de que se puedan revisar otros expedientes. Las
# estrategias conservan el mejor candidato parcial y el valor puede ampliarse
# explícitamente por entorno para investigaciones forenses.
WEB_DEGREE_TIMEOUT_SECONDS = _safe_float("CRAWLER_WEB_DEGREE_TIMEOUT", 60.0)
...
# Descubrimiento opcional mediante buscador: sólo genera candidatos; la
# identidad y la completitud se validan después con el pipeline habitual.
WEB_SEARCH_DISCOVERY_ENABLED = os.getenv(
    "CRAWLER_WEB_SEARCH_DISCOVERY_ENABLED", "1"
).strip().lower() not in {"0", "false", "no"}
WEB_SEARCH_DISCOVERY_ENDPOINT = os.getenv(
    "CRAWLER_WEB_SEARCH_DISCOVERY_ENDPOINT",
    "https://lite.duckduckgo.com/lite/",
).strip()
WEB_SEARCH_DISCOVERY_FALLBACK_ENDPOINTS = tuple(
    endpoint.strip().rstrip("/")
    for endpoint in os.getenv(
        "CRAWLER_WEB_SEARCH_DISCOVERY_FALLBACK_ENDPOINTS",
        "https://www.bing.com/search?format=rss",
    ).split(",")
    if endpoint.strip()
)
WEB_SEARCH_DISCOVERY_MAX_QUERIES = _safe_int(
    "CRAWLER_WEB_SEARCH_DISCOVERY_MAX_QUERIES", 2
)
WEB_SEARCH_DISCOVERY_MAX_RESULTS = _safe_int(
    "CRAWLER_WEB_SEARCH_DISCOVERY_MAX_RESULTS", 6
)
WEB_SEARCH_DISCOVERY_TIMEOUT = _safe_float(
    "CRAWLER_WEB_SEARCH_DISCOVERY_TIMEOUT", 12.0
)
# Evidencia de URLs académicas descubiertas en campañas anteriores. Se
# reutiliza sólo durante un intervalo acotado: así se evita perder hallazgos
# válidos entre ejecuciones sin convertir una URL antigua en una fuente eterna.
ACADEMIC_DISCOVERY_EVIDENCE_TTL_SECONDS = _safe_int(
    "CRAWLER_ACADEMIC_DISCOVERY_EVIDENCE_TTL",
    str(30 * 24 * 3600),
)
...
HUB_AND_SPOKE_MIN_PRIORITY = _safe_int("CRAWLER_HUB_MIN_PRIORITY", 40)     # Evita gastar el presupuesto en navegación genérica
# El número de hubs no limita los enlaces que cada página puede aportar al
# índice. Estas cotas evitan que un portal con navegación masiva convierta el
# mapa de candidatos en una estructura sin límite de memoria, sin depender de
# ninguna universidad o dominio concreto.
HUB_AND_SPOKE_MAX_INDEXED_URLS = _safe_int("CRAWLER_HUB_MAX_INDEXED_URLS", 12000)
HUB_AND_SPOKE_MAX_LINKS_PER_TOKEN = _safe_int("CRAWLER_HUB_MAX_LINKS_PER_TOKEN", 96)
...
# Recuperación genérica de resoluciones cuando la ficha RUCT no conserva el
# enlace al BOE. Sólo descubre referencias del buscador oficial; el documento
# sigue sometido a identidad, nivel y calidad curricular antes de persistirse.
BOE_SEARCH_DISCOVERY_ENABLED = os.getenv(
    "CRAWLER_BOE_SEARCH_DISCOVERY_ENABLED", "1"
).strip().lower() not in {"0", "false", "no"}
BOE_SEARCH_DISCOVERY_MAX_QUERIES = _safe_int(
    "CRAWLER_BOE_SEARCH_DISCOVERY_MAX_QUERIES", 2
)
BOE_SEARCH_DISCOVERY_MAX_RESULTS = _safe_int(
    "CRAWLER_BOE_SEARCH_DISCOVERY_MAX_RESULTS", 8
)
BOE_SEARCH_DISCOVERY_MAX_DOCUMENTS = _safe_int(
    "CRAWLER_BOE_SEARCH_DISCOVERY_MAX_DOCUMENTS", 6
)
BOE_SEARCH_DISCOVERY_DELAY = _safe_float(
    "CRAWLER_BOE_SEARCH_DISCOVERY_DELAY", 0.35
)
# La API oficial del sumario diario permite reconstruir referencias BOE cuando
# una ficha RUCT conserva la fecha pero no conserva el enlace al documento.
# La respuesta se vuelve a someter a las mismas puertas de identidad y calidad
# que cualquier otra fuente; estos límites evitan descargar diarios completos
# sin una señal temporal ya disponible.
BOE_SUMMARY_DISCOVERY_ENABLED = os.getenv(
    "CRAWLER_BOE_SUMMARY_DISCOVERY_ENABLED", "1"
).strip().lower() not in {"0", "false", "no"}
BOE_SUMMARY_DISCOVERY_MAX_DATES = _safe_int(
    "CRAWLER_BOE_SUMMARY_DISCOVERY_MAX_DATES", 2
)
BOE_SUMMARY_DISCOVERY_MAX_ITEMS = _safe_int(
    "CRAWLER_BOE_SUMMARY_DISCOVERY_MAX_ITEMS", 40
)
BOE_SUMMARY_DISCOVERY_DELAY = _safe_float(
    "CRAWLER_BOE_SUMMARY_DISCOVERY_DELAY", 0.35
)
...
# Importación de léxico y diccionarios de dominio
