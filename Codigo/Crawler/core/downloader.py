...
class RUCTDownloader:
    ...
    def __init__(self, delay=REQUEST_DELAY, max_retries=MAX_RETRIES, timeout=HTTP_TIMEOUT, metrics_tracker=None, respect_robots=RESPECT_ROBOTS, ledger=None, phase="", enable_http2=ENABLE_HTTP2):
        ...
        self.current_univ_name = ""
        self.current_univ_base_url = ""
        self.last_final_url = ""
        self._run_response_memo = OrderedDict()
        self._run_response_memo_bytes = 0

    def reset_university_context(self, univ_code: str, univ_name: str = "", base_url: str = ""):
        self.current_univ_code = univ_code
        self.current_univ_name = str(univ_name or "")
        self.current_univ_base_url = str(base_url or "")
        self.last_final_url = ""
        self.consecutive_failures = 0
        self.pause_count_univ = 0
    ...
    def _sleep_with_degree_budget(self, seconds: float) -> None:
        """Espera respetando el presupuesto acumulado de la titulación.

        Un ``Crawl-delay`` legítimo puede ser mayor que el tiempo disponible
        para una titulación. La espera sigue siendo cortés, pero no debe dejar
        un worker dormido indefinidamente ni impedir que la fase se cierre de
        forma reanudable.
        """
        ...
        if budget <= 0:
            time.sleep(delay)
            return
        remaining = budget - (time.monotonic() - self._degree_started_at)
        if remaining <= 0:
            raise DegreeTimeoutException(
                f"Titulación '{self.current_degree_code}' excede el presupuesto "
                f"acumulado de {budget:.1f}s"
            )
        if delay > remaining:
            time.sleep(remaining)
            raise DegreeTimeoutException(
                f"Titulación '{self.current_degree_code}' agotó su presupuesto "
                f"durante la espera de cortesía de {delay:.1f}s"
            )
        time.sleep(delay)
    ...
    def _apply_delay(self, url: str = ""):
        ...
        try:
            ...
            if elapsed < effective_delay:
                self._sleep_with_degree_budget(effective_delay - elapsed)
    ...
    def _is_transient_network_error(error: Exception) -> bool:
        ...
        return any(marker in text for marker in (
            ...
            "illegal status line", "badstatusline", "bad status line",
            "remotedisconnected",
        ))
            ...
            for target_url in urls_to_try:
                ...
                try:
                    ...
                    try:
                        ...
                        if any(marker in err_msg for marker in ("illegal status line", "badstatusline", "bad status line")) and HTTPX_AVAILABLE:
                            logger.info("[FALLBACK HTTP/2] Detectado error de status line HTTP/1.1 (%s). Reintentando con httpx HTTP/2...", req_err)
                            temp_client = None
                            try:
                                client = self.httpx_client
                                if client is None:
                                    temp_client = httpx.Client(
                                        http2=bool(HTTP2_AVAILABLE),
                                        timeout=httpx.Timeout(self.timeout, connect=WEB_CONNECTIVITY_TIMEOUT),
                                        follow_redirects=True,
                                        verify=False,
                                    )
                                    client = temp_client
                                req = client.build_request("GET", target_url, headers=request_headers or None)
                                httpx_resp = client.send(req, stream=stream)
                                response = HTTP2ResponseWrapper(
                                    httpx_resp,
                                    target_url,
                                    max_duration=HTTP_RESPONSE_MAX_DURATION_SECONDS,
                    ...
                    if final_url and target_url and not is_same_or_subdomain(final_url, target_url):
                        if not is_trusted_institutional_redirect(target_url, final_url, self.current_univ_name):
                            response.close()
                        ...
                        if self.respect_robots and not final_url.rstrip('/').lower().endswith('/robots.txt'):
                            final_allowed, final_delay = self.robots_policy.check(final_url)
                            if not final_allowed:
                                response.close()
                                reason = self.robots_policy.explain(final_url)
                                raise RobotsDeniedException(final_url, reason, context="destino de redirección institucional")
                            if final_delay is not None:
                                with self._LOCK_CREATION_LOCK:
                                    final_domain = urlparse(final_url).netloc.lower()
                                    self._GLOBAL_DOMAIN_DELAYS[final_domain] = max(
                                        self._GLOBAL_DOMAIN_DELAYS.get(final_domain, self.delay),
                                        min(max(0.0, final_delay), 3600.0),
                                    )
                        logger.info(
                            "[REDIRECCIÓN INSTITUCIONAL] %s -> %s aceptada para %s.",
                            target_url,
                            final_url,
                            self.current_univ_name or self.current_univ_code,
                        )
                    self.last_final_url = final_url or target_url
                    ...
                    if self.ledger is not None and getattr(response, "_unihub_cached", False) is not True:
                        try:
                            # ``record_attempt`` usa la URL normalizada de la
                            # solicitud; una variante www/HTTPS puede ser
                            # distinta y no debe dejar la fila original en
                            # estado ``processing``.
                            self.ledger.record_response(url, response=response, status="success")
                        except Exception as error:
                    ...
                    # Auto-freno proactivo de cortesía según cabeceras estándar de rate-limit
                    rl_remaining = response.headers.get("X-RateLimit-Remaining") or response.headers.get("RateLimit-Remaining")
                    if rl_remaining is not None:
                        try:
                            if int(rl_remaining) <= 2:
                                domain = urlparse(target_url).netloc.lower() if target_url else "default"
                                with self._LOCK_CREATION_LOCK:
                                    current = self._GLOBAL_DOMAIN_DELAYS.get(domain, self.delay)
                                    self._GLOBAL_DOMAIN_DELAYS[domain] = min(current * 1.5, ADAPTIVE_BACKOFF_MAX_DELAY)
                                logger.info("[AUTO-FRENO DE CORTESÍA] Límite bajo en '%s' (restantes: %s). Ajustando retardo a %.2fs.", domain, rl_remaining, self._GLOBAL_DOMAIN_DELAYS[domain])
                    ...
                    if self.ledger is not None:
                        try:
                            # Cerrar siempre la misma clave usada por
                            # ``record_attempt`` evita intentos huérfanos
                            # cuando el ciclo prueba una variante de URL.
                            self.ledger.record_response(url, response=locals().get("response"), status="failed", error=str(e))
    ...
    def fetch_text(
        ...
        url: str,
        encoding="utf-8",
        max_size_bytes: int = MAX_TEXT_RESPONSE_SIZE_BYTES,
        request_headers: dict | None = None,
    ) -> str:
        """Fetches decoded string content from a URL with charset detection."""
        with self._request_with_retry(url, stream=True, extra_headers=request_headers) as response:
            header_charset = None
