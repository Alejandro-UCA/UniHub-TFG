...
def _source_is_historical(url: str) -> bool:
    """Detecta rutas que normalmente describen planes no vigentes."""
    lowered = str(url or "").lower()
    return any(marker in lowered for marker in (
        "extingu", "extincion", "extinció", "ext-plan", "ext_", "historico", "historical",
    )) or bool(re.search(r"/20\d{2}-\d{2}/", lowered))
...
def assess_plan_quality(payload: dict, source_type: str | None = None) -> dict:
    ...
    if any(marker in source_type for marker in ("web_oficial", "centro_adscrito", "interuniversitario")):
        source_url = payload.get("web_fuente_directa_url") or payload.get("boe_url") or payload.get("web")
    else:
        source_url = payload.get("boe_url") or payload.get("web_fuente_directa_url") or payload.get("web")
    if not source_url:
        source_url = next(
            (
                item.get("url")
    ...
    if not source_url:
        source_url = payload.get("web")
    if not source_type:
        if payload.get("boe_url"):
            source_type = "resolucion_boe"
        elif payload.get("web") or payload.get("web_fuente_directa_url"):
            source_type = "web_oficial"
    if source_url:
        parsed_source = urlsplit(str(source_url))
        if parsed_source.scheme not in {"http", "https"} or not parsed_source.netloc:
            issues.append("fuente_url_invalida")
        elif _source_is_historical(source_url):
            issues.append("fuente_historica_o_plan_extinguido")
    else:
        issues.append("fuente_no_trazable")

    completeness = get_curriculum_completeness_status(payload)
    extraction_diagnostic = plan.get("diagnostico_extraccion") if isinstance(plan, dict) else None
    if isinstance(extraction_diagnostic, dict) and (
        plan.get("requiere_revision_parser")
        or extraction_diagnostic.get("detalle_curricular_suficiente") is False
    ):
        issues.append("extraccion_pdf_incompleta")

    is_doctorate = is_doctorate_program(payload.get("nivel_academico", ""), payload.get("titulo", ""))
    result = {
        ...
        "ects_totales_listados": completeness.get("total_ects_listed", 0.0),
        "ects_optativos_ofertados": completeness.get("total_ects_optativos_ofertados", 0.0),
        ...
        "verificaciones": {
            "fuente_url_presente": bool(source_url),
            "fuente_tipo_declarado": bool(source_type),
            "identidad_sin_conflicto": not any(issue.startswith("titulo_") for issue in issues),
            "fuente_no_historica": not any(issue == "fuente_historica_o_plan_extinguido" for issue in issues),
            "detalle_curricular_suficiente": (completeness["total_elementos"] > 0) or (is_doctorate and completeness["is_complete"]),
            "optatividad_resuelta": not completeness.get("optatividad_no_resuelta", False),
        },
    }
    if not is_doctorate and (not isinstance(plan, dict) or not plan):
        return result
    if issues:
    ...
    if is_doctorate and completeness["is_complete"]:
...
def apply_plan_quality(payload: dict, candidate_plan: dict | None, source_type: str | None = None) -> dict:
    ...
    previous_assessment = (
        assess_plan_quality(payload, source_type)
        if isinstance(previous_plan, dict) and previous_plan
        else None
    )
    # Una ejecución puede no aportar evidencia nueva (por ejemplo, la ficha
    # RUCT no contiene BOE) y, aun así, conservar un plan ya verificado por la
    # universidad. Evaluar ``None`` en ese caso marcaba calidad falsa aunque el
    # currículo persistido siguiera intacto. Sólo se reutiliza el plan previo
    # cuando la nueva entrada es realmente nula; una candidata no publicable
    # continúa aislada como antes.
    evaluation_plan = (
        previous_plan
        if candidate_plan is None and isinstance(previous_plan, dict)
        else candidate_plan
    )
    payload["plan_estudios"] = evaluation_plan
    assessment = assess_plan_quality(payload, source_type)
    if assessment.get("publicable"):
        synchronize_plan_quality_metadata(evaluation_plan, assessment)
    ...
    if previous_assessment and previous_assessment.get("publicable"):
        # La candidata fallida queda aislada para revisión, pero no puede
        # degradar la calidad del plan ya publicado ni crear una discrepancia
        # entre ``calidad_datos`` y el currículo persistido.
        payload["calidad_datos"] = previous_assessment
        payload["estado_calidad"] = previous_assessment["estado"]
    if not is_publishable_quality_status(payload.get("estado_calidad")):
        payload["estado_calidad"] = QUALITY_NO_VERIFIED_DATA
    return assessment


def promote_verified_candidate(payload: dict) -> dict:
    """Promueve un candidato ya persistido sólo si supera el control completo.

    Esta reconciliación cubre candidatos generados por una fase anterior que no
    volvieron a pasar por ``apply_plan_quality``. Es deliberadamente genérica:
    usa únicamente la identidad, procedencia y reglas de completitud del
    registro actual. Nunca reemplaza un plan que ya contenga detalle
    curricular, para evitar pérdidas silenciosas fuera del flujo de historial.

    Devuelve un diagnóstico con ``promoted`` para que las campañas puedan
    auditar tanto las promociones como los candidatos que siguen pendientes.
    """
    if not isinstance(payload, dict):
        return {"promoted": False, "reason": "payload_no_objeto"}

    candidate = payload.get("candidato_plan_estudios")
    if not isinstance(candidate, dict) or not candidate:
        return {"promoted": False, "reason": "sin_candidato"}

    current = payload.get("plan_estudios")
    current_has_detail = isinstance(current, dict) and bool(
        current.get("elementos_curriculares")
    )
    if current_has_detail:
        current_assessment = assess_plan_quality(payload, payload.get("origen_fuente"))
        if current_assessment.get("publicable"):
            return {
                "promoted": False,
                "reason": "plan_actual_ya_publicable",
                "assessment": current_assessment,
            }
        return {
            "promoted": False,
            "reason": "plan_actual_con_detalle_preservado",
        }

    candidate_payload = dict(payload)
    candidate_payload["plan_estudios"] = candidate
    assessment = assess_plan_quality(
        candidate_payload,
        payload.get("origen_fuente"),
    )
    synchronize_plan_quality_metadata(candidate, assessment)
    if not assessment.get("publicable"):
        return {
            "promoted": False,
            "reason": "candidato_no_publicable",
            "assessment": assessment,
        }

    payload["plan_estudios"] = deepcopy(candidate)
    payload.pop("candidato_plan_estudios", None)
    payload["calidad_datos"] = assessment
    payload["estado_ultima_extraccion"] = assessment["estado"]
    payload["estado_calidad"] = assessment["estado"]
    payload["estado_fuente"] = "verificada"
    return {
        "promoted": True,
        "reason": "candidato_verificado_promovido",
        "assessment": assessment,
    }


def synchronize_plan_quality_metadata(plan: dict | None, assessment: dict | None) -> None:
    """Mantiene coherentes los metadatos persistidos con la evaluación actual."""
    if not isinstance(plan, dict) or not isinstance(assessment, dict):
        return
    for plan_field, quality_field in (
        ("plan_completo", "plan_completo"),
        ("ects_totales_detectados", "ects_totales_detectados"),
        ("ects_totales_listados", "ects_totales_listados"),
        ("ects_optativos_ofertados", "ects_optativos_ofertados"),
        ("ects_exigidos", "ects_exigidos"),
        ("optatividad_no_resuelta", "verificaciones"),
    ):
        if plan_field == "optatividad_no_resuelta":
            plan[plan_field] = not bool(assessment.get("verificaciones", {}).get("optatividad_resuelta"))
        elif quality_field in assessment:
            plan[plan_field] = assessment[quality_field]
