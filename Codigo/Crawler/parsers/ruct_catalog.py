...
def classify_registry_entity(name: str) -> str:
    """Clasifica filas RUCT que no son universidades activas ordinarias."""
    lowered = sanitize_string_value(name).lower()
    if "centros extranjeros autorizados" in lowered:
        return "centro_extranjero_autorizado"
    if "centros dependientes del ministerio de defensa" in lowered:
        return "centro_dependiente_defensa"
    if "otros centros de nivel universitario" in lowered:
        return "otros_centros_universitarios"
    return "universidad"
    ...
    for tr in table.find_all("tr"):
        ...
        if code and name:
            universities.append({
                ...
                "entidad_ruct": classify_registry_entity(name),
            })
    return universities
...
def parse_degrees_from_html(html_text: str) -> list[dict]:
    ...
    for tr in table.find_all("tr"):
        ...
        if code and title:
            raw_rows.append({"code": code, "title": title, "nivel": nivel, "estado": estado, "row_dict": row_dict})
    return raw_rows
...
def extract_participating_universities(title: str, row_dict: dict | None = None) -> list[str]:
    """Detecta y extrae las universidades que integran un consorcio interuniversitario."""
    results = []
    seen = set()

    if row_dict and isinstance(row_dict, dict):
        for k, v in row_dict.items():
            if any(marker in str(k).lower() for marker in ("participante", "consorcio", "otras univ")):
                val = str(v or "").strip()
                if val:
                    for part in re.split(r"[,;\n]+", val):
                        clean_part = sanitize_string_value(part)
                        if clean_part and clean_part.lower() not in seen:
                            seen.add(clean_part.lower())
                            results.append(clean_part)

    clean_title = str(title or "").strip()
    match = re.search(
        r"\b(?:por\s+las?|por\s+els?|por\s+los?|per\s+les?|per\s+la|pola)\s+([Uu]niversi[^\n;]+)",
        clean_title,
        re.I,
    )
    if match:
        ...
        segments = re.split(
            r",\s*(?:la|el|les|els|las)?\s*|\s+(?:y|e|i|and)\s+(?:la|el|les|els|las)?\s*|;\s*",
            clause,
            flags=re.I,
        )
        for seg in segments:
            seg = seg.strip(" .;,")
            if not re.search(r"\b(?:universi|school|college|institute|institut)", seg, re.I):
                if any(kw in seg.lower() for kw in ("politécnica", "politecnica", "autónoma", "autonoma", "complutense", "carlos iii", "alcalá", "alcala", "sevilla", "zaragoza", "valència", "valencia")):
                    seg = f"Universidad {seg}"
                else:
                    continue
            clean_univ = sanitize_string_value(seg)
            if clean_univ and clean_univ.lower() not in seen:
                seen.add(clean_univ.lower())
                results.append(clean_univ)
...
def parse_degrees_xls(filepath: str) -> list[dict]:
    ...
    if is_html_fallback:
        ...
        for r in range(1, sheet.nrows):
            ...
            candidate_rows.append({"code": code, "title": title, "nivel": nivel, "estado": estado, "row_dict": row_dict})
    ...
    for row_data in candidate_rows:
        ...
        participantes = extract_participating_universities(title, row_data.get("row_dict"))
        is_inter = bool(participantes or "interuniversitari" in title_norm)
        if (
            ...
            if is_inter:
                deg_record["interuniversitario"] = True
                deg_record["universidades_participantes"] = participantes
            raw_active_degrees.append(deg_record)
