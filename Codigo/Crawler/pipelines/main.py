...
def _run_part(
    ...
    *,
    ...
    degree_level_filter: Optional[str] = None,
    ...
    if part in (2, 4):
        runner_kwargs["degree_level_filter"] = degree_level_filter
    if part == 4:
...
def run_phase1(
    ...
    *,
    ...
    if clean_runtime:
        from utils.data_cleanup import clean_crawler_runtime_data
...
Ejemplos de uso:
  ...
  python main_fase_1.py --parte 4 --univs 025 --grado "Medicina"
    ...
    parser.add_argument(
        "--nivel-titulacion", "--degree-level",
        choices=("master", "grado", "doctorado"),
        default=None,
        help="Filtrar por nivel académico de forma genérica.",
    )
    ...
    parser.add_argument(
        "--clean", "--clean-runtime",
        action="store_true",
        help="Limpiar de forma segura los artefactos volátiles (cachés SQLite, logs, planes) preservando las 3 semillas maestras antes de ejecutar.",
    )
    parser.add_argument(
        "--clean-dry-run",
        action="store_true",
        help="Simular la limpieza de artefactos volátiles sin eliminar ningún fichero ni iniciar el rastreo.",
    )
...
def main(argv=None) -> int:
    ...
    if args.clean_dry_run:
        from utils.data_cleanup import clean_crawler_runtime_data
        print("\n [SIMULACIÓN DE LIMPIEZA (--clean-dry-run)]")
        clean_crawler_runtime_data(dry_run=True)
        return 0
    ...
    result = run_phase1(
        ...
        degree_level=args.nivel_titulacion,
        clean_runtime=args.clean,
    )
