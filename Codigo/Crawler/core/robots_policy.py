...
from core.config import (
    ...
    ROBOTS_ERROR_CACHE_TTL_SECONDS,
...
class RobotsPolicy:
    ...
    def _evict_if_needed(cls):
        if len(cls._cache) > cls._MAX_CACHE_SIZE:
            if cls._last_fetch:
                ...
                cls._negative_cache.pop(oldest_origin, None)
                cls._sitemaps.pop(oldest_origin, None)
        if len(cls._negative_cache) > cls._MAX_CACHE_SIZE:
            oldest_negative = min(
                cls._negative_cache,
                key=lambda key: cls._negative_cache[key][0],
            )
            cls._negative_cache.pop(oldest_negative, None)

    @classmethod
    def _remember_negative(cls, origin: str, outcome: str) -> None:
        """Cachea una denegacion tecnica sin relajar el modo fail-closed."""
        if ROBOTS_ERROR_CACHE_TTL_SECONDS <= 0:
            return
        with cls._lock:
            cls._negative_cache[origin] = (time.time(), outcome)
            cls._last_outcome[origin] = outcome
    ...
    def _load(self, origin: str, robots_url: str) -> urllib.robotparser.RobotFileParser | None:
        ...
        with fetch_lock:
            ...
            try:
                ...
                parser.parse(raw_lines)
                
                # RFC 9309 §2.4: Extraer directivas oficiales Sitemap
                discovered_sitemaps = []
                for r_line in raw_lines:
                    r_clean = r_line.strip()
                    if r_clean.lower().startswith("sitemap:"):
                        sm_candidate = r_clean[8:].strip()
                        if sm_candidate.startswith(("http://", "https://")):
                            discovered_sitemaps.append(sm_candidate)

                with self._lock:
                    self._cache[origin] = (time.time(), parser)
                    self._sitemaps[origin] = discovered_sitemaps
                    self._evict_if_needed()
                    suffix = f"_tras_{redirect_count}_redirecciones" if redirect_count else ""
    ...
    def check(self, url: str) -> tuple[bool, float | None]:
        ...
        now = time.time()
        with self._lock:
            negative = self._negative_cache.get(origin)
            if negative and now - negative[0] < ROBOTS_ERROR_CACHE_TTL_SECONDS:
                self._last_outcome[origin] = f"cacheado_negativo:{negative[1]}"
                return (False if ROBOTS_FAIL_CLOSED else True), None
            if negative:
                self._negative_cache.pop(origin, None)
        ...
        if parser is None:
            ...
            if host in ROBOTS_CONFIRMED_NO_FILE_HOSTS and outcome.startswith("error_red_"):
                ...
                with self._lock:
                    outcome = self._last_outcome.get(origin, "robots_no_verificable")
                self._remember_negative(origin, outcome)
                return (False if ROBOTS_FAIL_CLOSED else True), None
        ...
        try:
            ...
            if delay_value is not None:
                # Delimitación de cortesía: acotar techos excesivos para proteger la ejecución (RFC 9309)
                delay_value = min(60.0, max(0.0, delay_value))
        except (TypeError, ValueError):
            delay_value = None
        return allowed, delay_value

    def get_sitemaps(self, url_or_origin: str) -> list[str]:
        """Devuelve las URLs de Sitemap declaradas oficialmente en robots.txt (RFC 9309 §2.4)."""
        resolved = self._origin(url_or_origin)
        if resolved is None:
            return []
        origin, robots_url = resolved
        now = time.time()
        with self._lock:
            cached = self._cache.get(origin)
            if cached and now - cached[0] < ROBOTS_CACHE_TTL_SECONDS:
                return list(self._sitemaps.get(origin, []))
        self._load(origin, robots_url)
        with self._lock:
            return list(self._sitemaps.get(origin, []))
