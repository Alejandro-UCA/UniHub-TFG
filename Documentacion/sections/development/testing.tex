...
\begin{itemize}
    ...
    \item \textbf{Widgets Dinámicos y Endpoints REST:} Detección agnóstica de atributos HTML5 (\texttt{data-config}, \texttt{data-url}), extracción de rutas REST y prevención SSRF contra dominios externos (\texttt{test\_generic\_dynamic\_widgets.py}).
    \item \textbf{Programas Oficiales de Doctorado (RD 99/2011):} Verificación de los tres patrones universales (listas, subtítulos, tablas), descarte de ruido administrativo, detección multilingüe de escuela y validación curricular sin créditos ECTS lectivos (\texttt{test\_doctoral\_extraction.py}).
...
\end{lstlisting}
El resultado de la verificación es de \textbf{407 pruebas superadas} y \textbf{una prueba omitida} de forma explícita (evaluación empírica con descarga de red en vivo), completándose la ejecución íntegra en apenas \textbf{37.3 segundos} con cero fallos y cero regresiones funcionales.
