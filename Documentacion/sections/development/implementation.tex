...
\begin{itemize}
    ...
    \item \textbf{Índice Hub-and-Spoke con BFS multinivel y Cola $O(1)$:} Preindexación en cascada mediante \texttt{collections.deque} para garantizar extracción en tiempo constante ($O(1)$ en lugar de $O(N)$ de listas estándar) sobre catálogos maestros, facultades y portales de calidad.
...
\begin{itemize}
    ...
    \item Dispone de un catálogo local pre-cargado \texttt{OFFICIAL\_SIIU\_PRICES\_CATALOG} para las 17 Comunidades Autónomas más la UNED con validación protegida contra excepciones de conversión numérica en floats/enteros.
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item \texttt{parse\_generic\_eees\_subject\_guide}: Interpreta bloques temáticos semánticos en HTML (Contenidos, Evaluación, Competencias, Bibliografía) con soporte multilingüe.
    ...
    \begin{enumerate}
        \item \textbf{Desglose Estructurado de Evaluación (\texttt{\_normalize\_evaluation\_breakdown}):} Extrae y normaliza los porcentajes ponderados de evaluación en cuatro métricas fijas: \texttt{teoria\_porcentaje}, \texttt{practicas\_porcentaje}, \texttt{evaluacion\_continua\_porcentaje} y \texttt{examen\_final\_porcentaje}.
